import numpy as np
import pandas as pd
from BR.Feature_Extraction.metric import IoU
...
def parse_outside_data(dti_dir,with_label=True, prot_len=2500, prot_vec=None,
                   drug_vec=None, drug_len=2048, protein_encoder=None, compound_encoder=None, return_dti=False,
                   pssm_json=None, return_sequence=False):
    dti_df = pd.read_csv(dti_dir)
    # print(drug_df)
    ...
    if compound_encoder is not None:
        ...
        print("Encoding compound with %s type"%drug_vec)
        #encoded_drug = [compound_encoder.encode(s) for s in smiles]
        dti_df["drug_feature"] = dti_df["Compound_ID"].progress_map(compound_encoder.encode)
        print(dti_df["drug_feature"][:10])
        print("Encoding compound ends!")

    #protein
    if prot_vec=="PSSM":
        protein_feature = dti_df["Protein_ID"].map(protein_encoder.encode).tolist()
    else:
        protein_feature = dti_df["Protein_ID"].map(protein_encoder.encode).tolist()
    #print(protein_feature[0:32])
    result_dic = {"protein_feature": protein_feature}
    ...
    if with_label:
        label = dti_df["Label"].values
        print("\tPositive data : %d" %(sum(dti_df["Label"])))
        print("\tNegative data : %d" %(dti_df.shape[0] - sum(dti_df["Label"])))
        result_dic.update({"label": label})

    if drug_vec.split("_")[0]=="SMILES":
        drug_feature = dti_df["Compound_ID"].map(compound_encoder.encode).tolist()
    else:
        drug_feature = np.stack(dti_df["drug_feature"])
    result_dic.update({"drug_feature": drug_feature})
